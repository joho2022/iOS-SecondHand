...
class UserManager: ObservableObject, UserProvider, UserLoginProvider, UserUpdateProvider, UserSignUpProvider, UserLocationProvider, UserFavoriteProvider {
    ...
    func addLikedProduct(_ productId: Int) {
        guard let user = user else { return }
        if !user.likedProducts.contains(productId) {
            user.likedProducts.append(productId)
            saveUser(user)
        }
    }
    ...
    func removeLikedProduct(_ productId: Int) {
        guard let user = user else { return }
        if let index = user.likedProducts.firstIndex(of: productId) {
            user.likedProducts.remove(at: index)
            saveUser(user)
        }
    }
}
