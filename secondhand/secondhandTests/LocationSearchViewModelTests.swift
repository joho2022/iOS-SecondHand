...
final class MockAddressService: AddressServiceProtocol {
    ...
    func fetchRoadAddresses(keyword: String, page: Int, completion: @escaping (Result<[String], secondhand.NetworkError>) -> Void) {
...
final class LocationSearchViewModelTests: XCTestCase {
    ...
    func test_SearchQueryUpdateResults() {
        ...
        let mockResults = ["경기도 수원시 권선구 호매실로 103 (호매실동, 호매실스타힐스)", "경기도 수원시 권선구 호매실로90번길 86 (호매실동)"]
    ...
    func test_LoadMoreResults() {
        ...
        let initialResults = ["경기도 수원시 권선구 호매실로 103 (호매실동, 호매실스타힐스)"]
        mockService.result = .success(initialResults)
        viewModel.searchQuery = "호매실"
        
        let moreResults = ["경기도 수원시 권선구 호매실로90번길 86 (호매실동)"]
